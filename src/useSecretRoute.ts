...
const fullSequence = ['up', 'up', 'down', 'down', 'left', 'right', 'left', 'right', 'b', 'a', 'start'];
const touchSequence = [...fullSequence.slice(0, 8), 'start'];
const keyboardKey: Record<string, string> = {
  ArrowUp: 'up', ArrowDown: 'down', ArrowLeft: 'left', ArrowRight: 'right',
  b: 'b', B: 'b', a: 'a', A: 'a', Enter: 'start',
};
const gamepadButtons: Array<[number, string]> = [
  [12, 'up'], [13, 'down'], [14, 'left'], [15, 'right'], [1, 'b'], [0, 'a'], [9, 'start'],
];
...
function sequenceMatcher(sequence: string[], onMatch: () => void) {
  ...
  return (input: string) => {
    if (input === sequence[index]) {
      index += 1;
      if (index === sequence.length) { index = 0; onMatch(); return true; }
      return false;
    }
...
export function useSecretRoute(onUnlock: () => void) {
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (event: TouchEvent) => {
      ...
      touchStart = null;
      const input = Math.max(Math.abs(x), Math.abs(y)) < 40
        ? 'start'
